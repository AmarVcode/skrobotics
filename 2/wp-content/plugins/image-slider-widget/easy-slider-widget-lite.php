...
add_action( 'current_screen', 'ewic_gutenberg_editor_test' );

function ewic_gutenberg_editor_test() {
	...
	if ( function_exists( 'get_current_screen' ) ) {
		...
		if ( method_exists( $current_screen, 'is_block_editor') && $current_screen->is_block_editor() ) {
			...
			add_filter( 'mce_external_plugins', 'ewic_gut_register' );
			add_filter( 'mce_buttons', 'ewic_register_buttons', 0 );
...
function ewic_gut_register( $plugins ) {
	
	$url = EWIC_URL_FILE . 'inc/js/gut_button.js';
	$plugins["ewic_mce"] = $url;
	return $plugins;
	
}

function ewic_register_buttons( $buttons ) {
	
	array_push( $buttons, 'ewic_mce' );
	return $buttons;
