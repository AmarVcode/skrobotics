...
if ( isset( $_GET['never_show_info'] ) ) {
		update_option('never_show_info', 0);
} else {
	$business_solution_pro_info = get_option('never_show_info', 1);
	if ($business_solution_pro_info > 0){ 
...
function business_solution_pro_admin_notice_info() {
    ...
    $message = __( 'Use Appearance > Customize > Theme Options to manage Sliders, Hero Content, Top Banner etc: ', 'business-solution' );
 	$dismiss = __( 'Always Hide this Notice', 'business-solution');
	...
	<em><a href="?never_show_info" target="_self"  class="dismiss-notice">%7$s</a></em> </p></div>', 
	esc_attr( $class ), 
	esc_html( $message ), 
